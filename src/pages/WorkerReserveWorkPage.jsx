    ...
    return (
        <div>
            ...
            <div className="mx-8 m-4 ">
                ...
                <p className="text-sm text-gray-500">1시간 단위로 알바 예약이 가능합니다.</p>
                <div className="flex items-center w-full my-4">
                    <AiOutlineCalendar className="mr-2" />
                    <p className="text-xs">{masage_date(state.order.date, 'korean')}</p>
                    {/* 직종 */}
                    <p className="text-xs ml-6 bg-gray-200 px-2 py-1 rounded-2xl">{state.order.type}</p>
                </div>
                <SelectBox selectedDate={selectedDate} getData={getWorkTime} data={workDates} mode={'RESERVE'} />
            </div>
            <div className="border-t-4 "></div>
            {/* 예약정보*/}
            <div className="mx-8 m-4 ">
